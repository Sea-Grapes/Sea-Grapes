# Hi, I'm Sea Grapes 👋

I like to build cool projects on the web. Sometimes they actually function properly.  
I am also very lazy and basically never finish anything lmao


<details open>
  <summary>✨ Github Stats</summary>

  ![General Stats](https://github-readme-stats.vercel.app/api?username=sea-grapes&show_icons=true&line_height=20&include_all_commits=true&theme=transparent)
  ![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Sea-Grapes&layout=compact&theme=transparent)
</details>
<details>
  <summary>🌊 random stuff i use</summary>
  Here's all the random tools that I use:

  ![My Stack](https://skillicons.dev/icons?i=js,ts,html,css,nodejs,svelte,react,tailwind,threejs,java,ai,ps,figma&theme=light)
</details>

Currently working on: website?

Come say hi at https://blurp.dev/chat 🙂